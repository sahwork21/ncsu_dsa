...
public class AdjacencyMatrixGraph<V, E> extends AbstractGraph<V, E> {
	/**The 2 d matrix of edges. Rows are outs cols are ins*/
    ...
    /**Keeps the index of the next place to add an edge*/
    ...
    public void removeVertex(Vertex<V> vertex) {
        GraphVertex v = validate(vertex);
        //We need to clear the row and column of the vertx's index
        int rowAndCol = validate(v).getIndex();
        //If the edge in the matr
        for(int i = 0; i < matrix[0].length; i++) {
        	//Directed method since the matrix will be symmetric so only row checks are needed
        	if(isDirected()) {
        		//We found an outgoing edge
        		if(matrix[rowAndCol][i] != null) {
        			edgeList.remove(matrix[rowAndCol][i].getPosition());
        			matrix[rowAndCol][i] = null;
        		}
        		
        		//We found an incoming edge
        		if(matrix[i][rowAndCol] != null) {
        			edgeList.remove(matrix[i][rowAndCol].getPosition());
        			matrix[i][rowAndCol] = null;
        		}
    ...
    public void removeEdge(Edge<E> edge) {
        GraphEdge e = validate(edge);
        //Just remove the edge and its locations in the matrix
        ...
        //Now get the coordinate and remove
