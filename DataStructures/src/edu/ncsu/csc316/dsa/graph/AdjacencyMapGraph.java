...
public class AdjacencyMapGraph<V, E> extends AbstractGraph<V, E> {
    ...
    public void removeVertex(Vertex<V> vertex) {
        ...
        //Get every key in the maps and remove the entry's from the adjacent vertices
        //Also remove from the overlying edgeList
        //Then remove from the vertexList
        for(Entry<Vertex<V>, Edge<E>> adjacent : v.getOutgoing().entrySet()) {
        	edgeList.remove(validate(adjacent.getValue()).getPosition());
        	validate(adjacent.getKey()).getIncoming().remove(v);
        }
        if(isDirected()) {
        	for(Entry<Vertex<V>, Edge<E>> adjacent: v.getIncoming().entrySet()) {
        		edgeList.remove(validate(adjacent.getValue()).getPosition());
        		validate(adjacent.getKey()).getOutgoing().remove(v);
        	}
        }
