...
public class StudentReader {
	...
	private static Student processLine(String line) {
		...
		//Creates the inputs for a Student
		String first = lineReader.next();
		String last = lineReader.next();
		String unityID = lineReader.next();
		int id = lineReader.nextInt();
		double gpa = lineReader.nextDouble();
		int creditHour = lineReader.nextInt();
		
		lineReader.close();
		return new Student(first, last, id, creditHour, gpa, unityID);
	}
}
