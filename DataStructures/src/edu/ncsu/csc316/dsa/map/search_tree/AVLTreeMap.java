 ...
 * performance for U+0007B;@see Map#put}, U+0007B;@see Map#get}, and U+0007B;@see Map#remove}.
 *
...
public class AVLTreeMap<K extends Comparable<K>, V> extends BinarySearchTreeMap<K, V> {
    ...
    private void rebalance(Position<Entry<K, V>> p) {
    	...
    	do {
        	...
        	if(!isBalanced(x)) {
        		//Get the taller child to be y
        		Position<Entry<K, V>> tallerChild = tallerChild(x);
        		//Get the taller grand child to be z
        		Position<Entry<K, V>> tallerGrandChild = tallerChild(tallerChild);
        		
        		//Restructure from the grandchild up
        		x = restructure(tallerGrandChild);
        	...
        	//Check if the parent is balanced
        	x = parent(x);
    	}
