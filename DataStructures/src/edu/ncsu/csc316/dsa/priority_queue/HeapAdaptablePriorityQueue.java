...
public class HeapAdaptablePriorityQueue<K extends Comparable<K>, V> extends HeapPriorityQueue<K, V>
    ...
    public static class AdaptablePQEntry<K, V> extends PQEntry<K, V> {
    	/**The index of this Entry in the array of Entries*/
        private int index;
     ...
     * keys
     * 
     * compare the custom Comparator to use when comparing keys (priorities)
    ...
    public void replaceKey(Entry<K, V> entry, K key) {
        //Replace the key and then bubble it to the right place
    	AdaptablePQEntry<K, V> pq = validate(entry);
    	pq.setKey(key);
    	bubble(pq.getIndex());
    }
