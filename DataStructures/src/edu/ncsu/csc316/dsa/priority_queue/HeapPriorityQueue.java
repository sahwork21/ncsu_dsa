...
public class HeapPriorityQueue<K extends Comparable<K>, V> extends AbstractPriorityQueue<K, V> {
    ...
    public Entry<K, V> insert(K key, V value) {
        ...
        //add at end of the ArrayList
        list.addLast(temp);
        
        //Need to upHeap at size - 1
        upHeap(list.size() - 1);
    ...
    public Entry<K, V> deleteMin() {
        ...
        //Swap the last and first elements then downHeap
        Entry<K, V> ret = list.get(0);
        swap(0, list.size() - 1);
        list.removeLast();
        downHeap(0);
        return ret;
    ...
    protected void upHeap(int index) {
        //Keep upHeaping until we have reached the parentIndex 0 or no swap was made
    	if(index > 0) {
    ...
    protected void downHeap(int index) {
        //Check if the current index is less than either children
    	...
    	if(hasLeft(index) && hasRight(index)) {
    		...
    		//The right is less than left so it is the smaller
    		if(compare(list.get(rightChild).getKey(), list.get(leftChild).getKey()) < 0) {
    			smallerChild = rightChild;
    		}
    		
    		//Check if a swap is needed. Swap if the smaller child comes before parent
    	...
    	//Need to only check the left
    	else if(hasLeft(index) && !hasRight(index) && compare(list.get(leftChild).getKey(), list.get(index).getKey()) < 0) {
    		//Check if a swap is needed. Swap if the smaller child comes before parent
    	...
    	//Otherwise we are at a leaf and no swaps are necessary
    }
}